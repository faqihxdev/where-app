    ...
    <div className='bg-white rounded-lg outline outline-1 outline-gray-200 overflow-hidden'>
      <div className='p-4'>
        ...
        <div className='mb-4'>
          <div className='bg-gray-100 rounded-lg p-2 flex items-center justify-between'>
            <div className='flex items-center space-x-2 flex-grow'>
              <img
                src={mainListing.images.main.data}
              ...
              <div>
                ...
                <div className='flex items-center text-xs text-gray-500'>
                  ...
                  <span className='truncate'>
                    {truncateWithEllipsis(mainListing.markers[0]?.name || 'No location', 20)}
                  </span>
                </div>
              </div>
            </div>
            <span
              className={`text-xs font-medium px-2 py-1 rounded-full ${mainListing.type === 'lost' ? 'bg-red-100 text-red-800' : 'bg-blue-100 text-blue-800'}`}>
        ...
        <div className='mt-4'>
          <div className='flex items-start space-x-4'>
            <img
              src={matchedListing.images.main.data}
            ...
            <div>
              ...
              <div className='flex items-center text-xs text-gray-500 mb-1'>
                <MapPinIcon className='h-3 w-3 mr-1' />
                <span className='truncate'>
                  {truncateWithEllipsis(matchedListing.markers[0]?.name || 'No location', 30)}
                </span>
              </div>
              <div className='flex items-center text-xs text-gray-500'>
      ...
      <div className='bg-gray-100 px-4 py-3 sm:px-6 flex justify-between items-center'>
        <div className='flex items-center space-x-2'>
          <span
            className={`text-xs font-medium px-2 py-1 rounded-full ${getMatchStatusColor(match.status)}`}>
            {match.status}
          </span>
          {distance !== null && (
            <span className='text-xs font-medium text-gray-500'>{distance} meters apart</span>
          )}
        </div>
        <div className='flex items-center gap-2'>
          ...
          <Menu>
            ...
            <MenuList>
              <MenuItem onClick={handleResolveMatch} icon={<CheckCircleIcon className='h-4 w-4' />}>
                Resolve
              </MenuItem>
              <MenuItem
